...
# pass is used syntatically so the code can be read. There are no operations or library references
# for this function. Mean averages of the message data base can be refined for future use
...
#  Method connection to SQL database. 
    with sqlite3.connect('MASTER.db') as conn: 
#  Initialization of curser object. 
        cursor = conn.cursor()
#  SQL inserts data into research table. 
        cursor.execute("""
...
# Insert_research function parameters. Creates a column for each. Adds a new row to the research table that contains the following columns. 
