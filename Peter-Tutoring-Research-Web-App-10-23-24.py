import streamlit as st
    ...
    #                                  Avoiding Conflicts: Simple unique module name that is not the same as another module. 
...
from langchain.llms import OpenAI
    # Allows you to import the OpenAI class from the Langchain library. (Allows you to import and utilize openAI language models.) 
    #langchain = library / framework. (Files within the langchain are called modules and modules are equivalent to a single file.)
from langchain.utilities import WikipediaAPIWrapper 
    # Imports a Langchain utilities. Purpose is to extract data from wikipedia specifically and use this to gain insights.  
...
from langchain.prompts import PromptTemplate
    ...
    #                                                      6. customization and control (Different AI model purpose = different prompt logic)
...
# Calls on different types of agents in the Langchain framework. This is contingent on the type of task we ask the model to perform. 
...
# Similar to code line 11 but is more generalized. It calls on the entire module that OpenAI provides as opposed to the specific .llms submodule 

from langchain.agents import initialize_agent
# Allows you to initialize an agent to be used in the langchain framework. (initialize is a function that allows the agent to use "tools" or classes within different modules to perform different tasks.)

from langchain.tools import DuckDuckGoSearchRun
# Allows agent to use DuckDuckGoSearchRun class to scrape data from different websites depending on the research topic at hand. 
...
# Vector store. Manage vectors of text data. (Used for similarity search and imrproving LLM.)
...
# and "from langchain.prompts import PromptTemplate" in providing context for the agent. This module specializes in questions and answers. 

from langchain.chains import VectorDBQA
# Similar to "from langchain.chains import RetrievalQA, ConversationalRetrievalChain" for Q and A. Utilizes vector database for pertinent data. 
...
def create_research_db():
    ...
    # Connects to the SQLite database and creates a new table named 'Research' if it doesn't exist. Each row is a new column in the table that is generated. 
        conn.execute("""
...
# The previous tools are initialized here and will be modified in streamlite in real time to see the output. 
...
class Document:
    # __init__ method needs to be indented inside the class
    def __init__(self, content, topic):
        # Inside __init__, these lines need to be indented
...
# the content and the topic when you create an "instance" "Basically another word for a specific object made using the class Document".
...
def main():
    ...
    # Sets streamlit page title. 
    create_research_db()
    # Research database is created. 
    llm=OpenAI(temperature=0.7)
    embedding_function = OpenAIEmbeddings()
    # Open AI instance is created with slightly high randomness.
    init_ses_states()
    # Session is initialized with this function call.
    if os.path.exists("./chroma_db"):
        st.session_state.embeddings_db = Chroma(persist_directory="./chroma_db", embedding_function=embedding_function)
    # If the embeddings / vector database are available it is accessed and loaded. 
    st.header("GPT-4 LangChain Agents Research Bot")
    # The String is presented as the header in streamilt. 
